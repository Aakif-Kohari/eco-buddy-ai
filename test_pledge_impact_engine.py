...
def _setup_test_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("green_pledge_tracker.DB_NAME", db_path)
    monkeypatch.setattr("pledge_leaderboard.DB_NAME", db_path)
    monkeypatch.setattr("pledge_impact_engine.DB_NAME", db_path)
    monkeypatch.setattr("database_connection.database_connection", lambda name: _ctx(name))
    yield db_path
...
class TestComparisonReport:
    ...
    def test_comparison_with_data(self, _init_tables):
        ...
        assert report.percentile_rank >= 0
        assert isinstance(report.vs_community, dict)

    def test_comparison_has_category_data(self, _init_tables):
        from pledge_impact_engine import generate_comparison_report
        report = generate_comparison_report(user_id=999)
        assert isinstance(report.category_comparison, list)
...
class TestFullReport:
    ...
    def test_generate_report_empty(self, _init_tables):
        ...
        assert report is not None
        assert report.user_id == 999
        assert report.period_weeks == 4
        assert report.generated_at
        assert isinstance(report.weekly_data, list)
    ...
    def test_generate_report_with_data(self, _init_tables):
        ...
        assert report.total_pledges_completed > 0
        assert report.total_co2_saved_kg > 0
        assert report.trend is not None
        assert report.prediction is not None
        assert len(report.insights) > 0
        assert len(report.milestones) > 0
        assert len(report.weekly_data) == 8
    ...
    def test_report_best_worst_week(self, _init_tables):
        ...
        if report.total_pledges_completed > 0:
            assert report.best_week is not None
            assert report.worst_week is not None
...
class TestIntegration:
    ...
    def test_full_workflow(self, _init_tables):
        """Test a complete user journey: enrol → complete → analyse → report."""
        from green_pledge_tracker import create_pledge, checkin_pledge, current_week_start
