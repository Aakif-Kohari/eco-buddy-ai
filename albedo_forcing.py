...
``src/lib/uhi_planner.py`` already knows that a dark roof reflects 12% of the
...
---------------------------------------------------------
``neighborhood_canopy_engine.py`` recommends planting on cooling and
...
that ``water_scarcity.py`` says is scarce. Reported as one "cooling benefit"
...
-------------------------------------------
*   ``src/lib/uhi_planner.py`` holds the local half of this and the same surface
    table. Nothing there is modified; this module carries its own values so the
    two can be compared rather than coupled.
*   ``neighborhood_canopy_engine.py`` gains a check it currently lacks.
*   ``abatement_curve.py`` can price these interventions once they are in
    tonnes, which is the main reason for doing the conversion at all.
*   ``climate_metrics.py`` handles well-mixed gases. A surface property is not a
...
SURFACES = {
    ...
    "grassland": {
        ...
        "note": "Cools mostly by evaporating, which requires water. Where that "
...
def globalise(local_forcing_w_m2, area_m2):
    ...
    is that so is one person's annual emissions.
    """
    if area_m2 <= 0:
...
def local_versus_global(result):
    ...
    return {
        ...
        "explanation": (
            ...
            "water. Both are worth having and they are not the same benefit, "
...
def abatement_cost(result, cost_per_m2, maintenance_cost_per_m2_yr=0.0,
    ...
    return {
        ...
        "note": (
            "Net warming, so there is no cost per tonne abated to report."
            if tonnes_abated <= 0 else
            "Comparable directly against the measures in abatement_curve.py."
        ),
    }
