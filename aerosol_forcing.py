...
``climate_metrics.py`` does the hard part of multi-gas accounting properly. It
...
order of magnitude in black carbon and barely at all in CO2. ``transport_planner.py``
and ``vehicle_emissions_data.py`` cannot presently tell them apart on climate
...
``climate_metrics.py`` reports and are labelled as short-lived throughout, so a
reader always knows which part of a figure will still be there in fifty years.

Where this connects to code already merged
-------------------------------------------
*   ``climate_metrics.py`` holds the long-lived half. This is the short-lived
    half, and the two are designed to be read together.
*   ``health_cobenefits.py`` already tracks PM2.5 for mortality. The same
...
*   ``emission_factors.py``, ``vehicle_emissions_data.py`` and
    ``transport_planner.py`` supply the activities; none of them is modified.
...
# health_cobenefits.py does this properly and this is a cross-reference, not a
# competing estimate.
...
def assess_activity(source, units, region=DEFAULT_REGION):
    ...
    return {
        ...
        "overlay_note": (
            ...
            "climate_metrics.py and do not redefine it. The CO2 line here is "
...
def unmasking(before_source, after_source, units=1.0, region=DEFAULT_REGION,
    ...
    return {
        ...
        "mortality_caveat": (
            ...
            "health_cobenefits.py does this properly; this is a cross-"
