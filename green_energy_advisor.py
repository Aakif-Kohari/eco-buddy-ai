...
def build_energy_advisor_report(
    ...
    """Build a comprehensive green energy advisor report."""
    grid = GRID_INTENSITY.get(region, GRID_INTENSITY["Global"])
...
def _generate_recommendations(
    ...
    """Generate prioritised energy recommendations."""
    recs: list[str] = []
    ...
    try:
        ...
        cursor = conn.execute(
            ...
            (
                ...
                report.monthly_kwh,
                report.region,
                report.best_solar.system_key if report.best_solar else None,
                report.best_battery.battery_key if report.best_battery else None,
                report.best_provider.provider_key if report.best_provider else None,
                report.total_annual_savings_potential,
                report.total_annual_co2_reduction_kg,
            ),
        )
        conn.commit()
