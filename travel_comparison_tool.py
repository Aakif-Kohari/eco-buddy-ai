...
class TravelComparisonEngine:
    ...
    def calculate_comparison_summary(self, results: Dict[TransportationMode, Dict[str, float]]) -> Dict[str, Any]:
        ...
        if best_overall[1]['environmental_score'] > 0.8:
            recommendations.append(f"Best overall: {best_overall[0].value} provides excellent environmental performance")
        else:
            ...
            if low_emission_mode[1]['total_emissions_kg'] < emission_avg * 0.5:
                recommendations.append(f"Consider {low_emission_mode[0].value} to reduce emissions by {int((1 - low_emission_mode[1]['total_emissions_kg']/emission_avg) * 100)}%")
        ...
        if low_cost_mode[1]['total_cost_usd'] < cost_avg * 0.8:
            recommendations.append(f"{low_cost_mode[0].value} is the most cost-effective option")
        ...
        if fastest_mode[1]['duration_minutes'] < time_avg * 0.8:
            recommendations.append(f"{fastest_mode[0].value} is the fastest option")
    ...
    def create_multi_modal_journey(self, segments: List[JourneySegment]) -> JourneyComparison:
        ...
        if environmental_score < 0.5:
            recommendations.append("Consider alternative modes with lower emissions")
        if cost_score < 0.5:
            recommendations.append("Look for more cost-effective transportation options")
        if time_score < 0.5:
            recommendations.append("Consider faster transportation options")
...
class TravelLogger:
    ...
    def _update_summary(self, user_id: str) -> None:
        ...
        if avg_co2_per_km > 0.15:
            recommendations.append("Consider using more sustainable transport options")
        if total_emissions > 1000:
            recommendations.append("Consider offsetting your travel emissions")
        if total_distance > 10000:
            recommendations.append("Try combining trips or using public transport more often")
...
class TravelComparisonTool:
    ...
    def generate_travel_report(self, user_id: str) -> str:
        ...
        report = []
        report.append("=" * 70)
        report.append(f"  TRAVEL IMPACT REPORT - USER: {user_id}")
        report.append("=" * 70)
        report.append(f"  Period: {self.travel_logger.get_user_summary(user_id).period_start.strftime('%Y-%m-%d')} to {self.travel_logger.get_user_summary(user_id).period_end.strftime('%Y-%m-%d')}")
        report.append("")
        report.append("  📊 TRAVEL SUMMARY")
        report.append(f"    Total Journeys: {stats['total_journeys']}")
        report.append(f"    Total Distance: {stats['total_distance_km']:,.1f} km")
        report.append(f"    Total Travel Time: {stats['total_duration_hours']:.1f} hours")
        report.append(f"    Total CO2 Emissions: {stats['total_co2_emissions_kg']:,.0f} kg")
        report.append(f"    Total Travel Cost: ${stats['total_cost_usd']:,.2f}")
        report.append("")
        report.append("  📈 KEY METRICS")
        report.append(f"    Average CO2 per km: {stats['average_co2_per_km_kg']:.3f} kg/km")
        report.append(f"    Average CO2 per hour: {stats['average_co2_per_hour_kg']:.2f} kg/hour")
        report.append(f"    Average Cost per km: ${stats['average_cost_per_km_usd']:.3f}")
        report.append(f"    Environmental Rating: {stats['environmental_impact_rating']}")
        report.append("")
        report.append("  🚗 MOST USED MODES")
        for mode, distance in stats['top_modes_used']:
            report.append(f"    {mode.value.replace('_', ' ').title()}: {distance:,.0f} km")
        report.append("")
