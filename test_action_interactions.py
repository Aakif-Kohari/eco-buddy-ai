...
def test_no_impact_in_combined_report_is_honest():
    ...
    assert not report.combined_impact.available
    assert "unavailable" in report.combined_impact.label.lower()
...
def test_completed_action_can_still_be_analyzed():
    ...
    assert report.selected_action_ids == ["insulate", "heat"]
