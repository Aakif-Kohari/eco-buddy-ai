...
Dependencies: green_pledge_tracker, pledge_leaderboard, database_connection.
"""
...
def _save_report(user_id: int, report: ImpactReport) -> None:
    ...
    with database_connection(DB_NAME) as conn:
        cur = conn.cursor()
        data = {
            "period_weeks": report.period_weeks,
            "total_co2_saved_kg": report.total_co2_saved_kg,
            "total_xp": report.total_xp,
            "total_pledges_completed": report.total_pledges_completed,
            "total_checkins": report.total_checkins,
            "avg_weekly_co2_kg": report.avg_weekly_co2_kg,
        }
        cur.execute(
            "INSERT INTO impact_reports (user_id, report_data, generated_at) VALUES (?, ?, ?)",
            (user_id, json.dumps(data, default=str), report.generated_at),
        )
        conn.commit()
...
def impact_report_to_dict(report: ImpactReport) -> dict[str, Any]:
    ...
    d = asdict(report)
    if report.best_week:
        d["best_week"] = asdict(report.best_week)
    if report.worst_week:
        d["worst_week"] = asdict(report.worst_week)
    if report.trend:
        d["trend"] = asdict(report.trend)
    if report.prediction:
        d["prediction"] = asdict(report.prediction)
    d["insights"] = [asdict(i) for i in report.insights]
    d["milestones"] = [asdict(m) for m in report.milestones]
    return d
