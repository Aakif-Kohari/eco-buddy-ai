...
def assess_analysis(
    ...
    if missingness > 0.25:
        recommendations.append("Complete missing fields before using high-confidence analytics.")
    if span < 90 and key == "forecast":
        recommendations.append("Collect a longer history before relying on forecasting.")
    if key == "comparison" and count == 2:
        recommendations.append("Add more assessments to make comparisons less sensitive to one record.")
    if not records:
...
def evidence_for_category(report: ReadinessReport, category: str) -> list[EvidenceRecord]:
    ...
    return [x for x in report.records if x.category == target]
...
def report_hash(report: ReadinessReport) -> str:
    payload = json.dumps(report.to_dict(), sort_keys=True, default=str).encode()
...
def persist_report(report: ReadinessReport, connection: sqlite3.Connection) -> int:
    ...
    cursor = connection.execute(
        """INSERT INTO analytics_readiness_reports
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (
            report.user_id, report.generated_at, report.engine_version,
            report.status, report.confidence, report_hash(report), report.to_json(),
        ),
    )
    connection.commit()
...
def readiness_matrix(report: ReadinessReport) -> list[dict[str, Any]]:
    return [
        ...
        for key, value in report.analyses.items()
    ]


def category_matrix(report: ReadinessReport) -> list[dict[str, Any]]:
    return [x.to_dict() for x in report.categories]
...
def explain_confidence(report: ReadinessReport) -> dict[str, Any]:
    return {
        ...
        "quality_breakdown": record_quality_breakdown(report.records),
        "coverage": data_coverage(report.records),
