...
def test_build_report_contains_all_sections():
    ...
    assert "combined_reduction" in report.metadata
...
def test_report_deduplicates_duplicate_conflicts():
    ...
    keys = [(item.conflict_type, item.goal_ids, item.title) for item in report.conflicts]
...
def test_report_metadata_records_validation_warnings():
    ...
    assert report.metadata["validation_warnings"]
...
def test_multiple_categories_are_independent():
    ...
    categories = {item.category for item in report.goals}
...
def test_overlapping_conflict_contains_conservative_overlap():
    ...
    overlap = next(item for item in report.conflicts if item.conflict_type == CONFLICT_OVERLAPPING_TARGET)
...
def test_unknown_category_is_reported():
    ...
    assert any(item.conflict_type == "UNSUPPORTED_CATEGORY" for item in report.conflicts)
...
def test_unsatisfied_dependency_generates_conflict():
    ...
    assert any(item.conflict_type == CONFLICT_DEPENDENCY for item in report.conflicts)
...
def test_multiple_critical_conflicts_raise_risk():
    ...
    assert any(item.status == UNLIKELY for item in report.goals)
...
def test_per_goal_risk_is_bounded():
    report = build_feasibility_report([goal()], [], as_of=BASE)
    assert 0 <= report.goals[0].risk_score <= 100
