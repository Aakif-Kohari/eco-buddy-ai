...
An additive analytics layer for EcoBuddy's existing reduction goals.  The
...
@dataclass(frozen=True)
class GoalConflict:
    """An explainable relationship between one or two goals."""
...
class GoalDependency:
    """A prerequisite relationship between goals."""
...
class FeasibilityReport:
    """Complete multi-goal feasibility report."""
...
def detect_duplicates(goals: Sequence[Mapping[str, Any]]) -> list[GoalConflict]:
    """Detect exact or near-exact duplicate goals."""
    conflicts: list[GoalConflict] = []
...
def build_recommendations(
    ...
    if any(item.conflict_type == CONFLICT_DUPLICATE_GOAL for item in conflicts):
        recommendations.append("Review duplicate goals and keep a single source of truth for each target.")
    if any(item.conflict_type == CONFLICT_OVERLAPPING_TARGET for item in conflicts):
        recommendations.append("Do not add reductions from overlapping goals as if they were independent savings.")
    if any(item.conflict_type == CONFLICT_IMPOSSIBLE_TIMELINE for item in conflicts):
        recommendations.append("Extend aggressive deadlines or reduce the requested target before committing to the pathway.")
    if any(not item.satisfied for item in dependencies):
        recommendations.append("Complete prerequisite goals before treating dependent goals as fully supported.")
    ...
    if any(result.status == INSUFFICIENT_DATA for result in results):
        recommendations.append("Collect additional dated assessments before relying on pace-based feasibility.")
    if not recommendations:
        recommendations.append("The current goal set has no detected critical feasibility blockers.")
...
def build_feasibility_report(
    ...
    # reversing the input goal order cannot change the report.
...
def report_id(report: FeasibilityReport) -> str:
    """Stable content-derived identifier for a report."""
...
def persist_feasibility_report(
    ...
    """Persist an immutable snapshot, replacing only an identical report."""
    ...
    cursor = connection.execute(
        ...
        (
            rid,
            None if report.user_id is None else str(report.user_id),
            report.analyzed_on.isoformat(),
