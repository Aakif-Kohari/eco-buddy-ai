...
def test_full_report_empty():
    ...
    assert set(report.module_results)==set(MODULES)
...
def test_report_json_roundtrip():
    ...
    assert restored["engine_version"]=="1.0" and restored["status"]==report.status
...
def test_nonfinite_full_report():
    ...
    assert any(x.code=="MISSING_METRIC_VALUE" for x in report.findings)
...
def test_future_full_report():
    ...
    assert any(x.code=="FUTURE_METRIC_DATE" for x in report.findings)
...
def test_module_record_counts():
    report=validate_all_modules({"analytics":[{"id":1,"metric":"x","category":"general","value":1,"unit":"count"}]})
    assert report.module_results["analytics"].record_count==1
    assert report.module_results["goals"].record_count==0
...
def test_missing_optional_modules_safe():
    ...
    assert report.module_results["goals"].record_count==3
...
def test_module_empty_status():
    report=validate_all_modules({})
    assert all(x.status==STATUS_INSUFFICIENT_DATA for x in report.module_results.values())

def test_large_mixed_input():
    rows=[{"id":i,"metric":"x","category":"transport","value":i+1,"unit":"count","date":"2026-01-01"} for i in range(75)]
    report=validate_all_modules({"analytics":rows})
    assert report.summary["total_records"]==75
...
def test_report_categories():
    ...
    assert report.summary["categories_seen"]==1

def test_report_modules_with_data():
    report=validate_all_modules({"analytics":[{"id":1,"metric":"x","category":"general","value":1,"unit":"count"}]})
    assert report.summary["modules_with_data"]==1
...
def test_cross_module_report_review():
    data={"recommendations":[{"id":1,"title":"Water","category":"water","estimated_impact":10}],
          "assessments":[assessment()]}
    report=validate_all_modules(data)
    assert report.status in {STATUS_REVIEW,STATUS_INCONSISTENT}
...
def test_cross_module_report_consistent():
    ...
    report=validate_all_modules(data)
    assert report.status in {STATUS_CONSISTENT,STATUS_REVIEW}
...
def test_findings_sorted_codes_in_full_report():
    ...
    assert report.findings==sorted(report.findings,key=lambda x:(x.code,x.module,x.message,x.record_ids)) or report.findings
