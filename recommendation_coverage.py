...
pipeline.  It does not generate new recommendations.  Instead it answers
...
class CoverageReport:
    """Complete, deterministic recommendation coverage report."""
...
def normalize_recommendation(
    ...
    *,
    ...
    This function intentionally accepts the output of both ``recommendations.py``
    and ``recommendation_engine.py`` so the coverage layer does not force a
...
def normalize_recommendations(
    ...
    *,
    ...
    """Normalize and deterministically order existing recommendations."""
    result = [
...
def _impact_bucket(share: float, config: RecommendationCoverageConfig) -> str:
    if share >= config.critical_impact_share:
        return "critical"
    if share >= config.high_impact_share:
        return "high"
    return "normal"
...
def _coverage_status(
    ...
    if effective >= config.full_coverage_recommendations:
        return CoverageStatus.COVERED
    if effective >= config.partial_coverage_recommendations:
...
def _gap_severity(
    ...
    *,
    ...
    if share >= config.critical_impact_share and completed_count > 0:
        return GapSeverity.CRITICAL
    if share >= config.critical_impact_share:
        return GapSeverity.CRITICAL
    if share >= config.high_impact_share:
        return GapSeverity.HIGH
    if rejected_count > 0 and share >= config.high_impact_share / 2:
...
def _coverage_score(
    ...
    effective = max(0, relevant_count - completed_count)
    count_score = min(1.0, effective / config.full_coverage_recommendations)
    repetition_penalty = min(0.35, repetition_rate * 0.35)
    rejection_penalty = min(0.25, rejected_count / max(relevant_count, 1) * 0.25)
    impact_bonus = 0.10 if share >= config.high_impact_share and effective > 0 else 0.0
...
def build_coverage_report(
    ...
    *,
    ...
    """Build the complete recommendation coverage report."""
    config = config or RecommendationCoverageConfig()
    normalized = mark_history_status(normalize_recommendations(recommendations), feedback)
    rows = analyze_category_coverage(contributors, normalized, feedback=feedback, config=config)
    repeated = detect_repeated_recommendations(normalized, threshold=config.repeated_title_threshold)
    ...
    high_uncovered = sum(
        1 for row in rows
        if row.impact_share >= config.high_impact_share and row.status in {CoverageStatus.GAP, CoverageStatus.PARTIAL}
    )
    ...
    report_metadata = {
        ...
        "high_impact_threshold": config.high_impact_share,
        "critical_impact_threshold": config.critical_impact_share,
...
def summarize_coverage(report: CoverageReport) -> dict[str, Any]:
    ...
    severity_counts = Counter(gap.severity.value for gap in report.gaps)
    return {
        ...
        "covered_categories": report.covered_category_count,
        "high_impact_uncovered": report.high_impact_uncovered_count,
        "diversity": report.recommendation_diversity,
    ...
    def save(self, report: CoverageReport) -> int:
        ...
        with self._connect() as connection:
            cursor = connection.execute(
                ...
                VALUES (?, ?, ?, ?, ?)""",
                (report.user_id, report.overall_score, report.status.value, payload, report.created_at),
            )
...
def category_coverage_index(report: CoverageReport) -> dict[str, CategoryCoverage]:
    return {row.category: row for row in report.categories}
